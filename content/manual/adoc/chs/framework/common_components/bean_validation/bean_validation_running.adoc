...
<<gui_DateField,DateField>>和<<gui_DatePicker,DatePicker>>组件使用 `@Past` 和 `@Future` 注解自动设置其 `rangeStart` 和 `rangeEnd` 属性，不过这里忽略了时间部分。
...
对于创建和更新操作, 通常 <<rest_api_v2,REST API>> 会自动执行 bean 验证。验证错误会以如下方式返回给客户端:
