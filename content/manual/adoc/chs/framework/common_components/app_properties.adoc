...
+
--
应用程序属性的值可以设置在<<app_properties_db,数据库>> 、 <<app_properties_files,属性文件>> ，也可以通过 Java 系统属性设置。此外，文件中设置的值将覆盖数据库中具有相同名称的值，通过 Java 系统属性设置的值将覆盖文件和数据库中的值。
...
[TIP]
====
当需要为平台属性设置值时，请在<<app_properties_reference,此文档>>中找到相关属性。如果文档声明该属性存储在数据库中，请使用 *Administration > Application Properties* 界面设置其值。否则，找出哪些 block 使用该属性并在这些 block 的 `app.properties` 文件中定义它。例如，如果文档声明该属性在所有 block 中使用，并且应用程序由中间件和 Web 客户端组成，则应在 *core* 模块的 `app.properties` 文件和 *web* 模块的 `web-app.properties` 文件中定义该属性。部署参数也可以在项目文件之外的<<conf_dir,配置>>目录中设置。有关详细信息，请参阅<<app_properties_files>>。
====
--
