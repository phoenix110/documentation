...
服务构成了一个层，在这一层定义了客户端<<app_tiers,层>>可用的一组中间层操作。换句话说，服务是中间层业务逻辑的入口点。在服务中，可以管理<<transactions,事务>>、检测用户<<permissions,授权>>、使用数据库或将操作委托给中间层的其它<<managed_beans,托管 Bean>>去执行。
