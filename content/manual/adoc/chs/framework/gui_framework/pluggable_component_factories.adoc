...
插件工厂机制扩展了标准组件的创建过程，允许在 <<gui_FieldGroup, FieldGroup>>、 <<gui_Table, Table>> 和 <<gui_DataGrid, DataGrid>> 中创建不同的编辑字段。这意味着应用程序组件或应用程序项目本身可以提供自定义策略，以创建非标准的组件或支持自定义数据类型。
...
* `componentClass` - 要创建的组件的类型。例如，`FieldGroup` 、 `Table` 、 `DataGrid`。
