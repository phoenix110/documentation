...
下面是构建应用程序并部署至 https://jelastic.com/[Jelastic] 云的示例。
...
* 如果创建的环境使用 PostgreSQL，你会收到一封带有数据库连接信息的 email。根据 email 里面的链接打开数据库管理界面，然后创建一个空数据库。数据库名称可以稍后在自定义的 `context.xml` 文件设置。
. 使用 CUBA Studio 构建 *Single WAR* 文件：
* 在主菜单选择 *CUBA > Deployment > WAR Settings*。
* 勾选 *Build WAR* 复选框。
* 在 *Application home directory* 字段填写 `${app.home}`。
+
[source, properties]
----
appHome = '..'
...
* 如果项目使用了 PostgreSQL，点击 *Custom context.xml path* 旁边的 *Generate* 按钮。设置数据库的用户名、密码、主机以及之前使用 email 提供的链接创建的数据库名。
+
...
* 在 *App properties* 字段填写 `${cuba.logDir}`：
+
[source, properties]
----
...
* 命令行运行 `buildWar` 启动构建过程。（假设之前已经创建了 Gradle wrapper）：
+
[source, plain]
----
gradlew buildWar
