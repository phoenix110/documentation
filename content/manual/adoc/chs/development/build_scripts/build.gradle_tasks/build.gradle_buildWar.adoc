...
* `appHome` – 指向<<app_home,应用程序主目录>>。可以使用绝对目录，或者相对于 home 目录的相对目录，亦或是 Java 系统变量的占位符，系统变量需要在服务启动时设置。
...
单个 WAR 包部署时所有的 sevlets 和 filters 需要通过编程的方式注册，参考<<servlet_registration_single_war,单一 WAR 包部署注册 Servlets>>。
