...
在 `context-param` 元素中，定义了<<app_components,应用程序组件>>列表和<<app_properties_files,应用程序属性文件>>列表。`webResourcesTs` 参数以及在构建时替换的这个参数的值，保证了在网页浏览器中能正确的缓存静态资源。
...
`CubaRestApiServlet` 提供全局的 <<rest_api_v2,REST API>>。
--
